...
class TestProxyHandler(unittest.TestCase):
    ...
    def test_handle_proxy_success(self, mock_requests_get):
        ...
        mock_response.text = "unit_test_ip"
    ...
    def test_process_proxies(self):
        """
        Test run on proxies_test.csv to see if it returns a result list.
        Both functions used are tested separately as well.
        :return: None
        """
        return_list = self.handler.process_proxies()
        comment = " - PROXY HANDLER - INFO - CURRENT WORKING PROXY LIST:"
